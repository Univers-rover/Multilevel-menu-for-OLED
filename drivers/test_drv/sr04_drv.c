...
static long sr04_ioctl(struct file *filp, unsigned int command, unsigned long arg)
{
	...
	switch (command)
	{
		case CMD_TRIG:
		{
			gpio_direction_output(gpios[1].gpio, 1);
			udelay(100);
...
/* 从ioctl开始计算超时，在中断中取消，如果超时未取消就在buffer中放入-1表示出错 */
...
static int __init sr04_init(void)
{
	...
	{		
		...
		err = request_irq(gpios[0].irq, sr04_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, gpios[0].name, &gpios[0]);
